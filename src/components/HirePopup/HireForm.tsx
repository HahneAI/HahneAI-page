...
export function HireForm({ onClose, selectedServiceId }: HireFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      {formError && (
        <div className="bg-primary-500/10 border border-primary-500/50 text-primary-400 px-4 py-2 rounded-lg text-sm">
          {formError}
        </div>
      )}
      ...
      <div>
        <label className="block text-sm font-medium text-neutral-300 mb-3">
          Services Interested In <span className="text-primary-500">*</span>
        </label>
        <div className="space-y-3">
          {services.map((service, index) => (
            <label key={service.id} className="flex items-center space-x-3">
              ...
              />
              <span className="text-neutral-300 text-sm">{highlightAITerms(service.title)}</span>
            </label>
          ))}
          ...
          <label className="flex items-center space-x-3 relative">
            <input
              ...
              className="w-4 h-4 rounded border-neutral-700 text-secondary-500 focus:ring-secondary-500 focus:ring-offset-neutral-900"
            />
            <span className="text-neutral-300 text-sm flex items-center space-x-2 group">
              <span className="bg-gradient-to-r from-primary-500 to-secondary-400 bg-clip-text text-transparent font-semibold">
                I.S.I Framework
              </span>
              <Sparkles
                size={14}
                className="text-secondary-400 opacity-50 group-hover:opacity-100 transition-opacity"
              />
            </span>
            <div className="absolute -left-2 -right-2 -top-2 -bottom-2 border border-secondary-500/20 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
          </label>
        </div>
      </div>
      ...
      <div>
        <label className="block text-sm font-medium text-neutral-300 mb-3">
          When would you like to schedule a meeting? <span className="text-primary-500">*</span>
        </label>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg border text-sm ${
              selectedSchedule === 'asap'
                ? 'border-secondary-500 bg-secondary-500/20 text-white'
                : 'border-neutral-700 text-neutral-300 hover:border-secondary-500/50'
            } transition-colors`}
          >
            ASAP
          </button>
      ...
      <button
        ...
        className="w-full bg-gradient-to-r from-primary-600 to-secondary-500 text-white px-4 py-3 rounded-lg hover:from-primary-700 hover:to-secondary-600 transition-all font-light tracking-wider disabled:opacity-50 disabled:cursor-not-allowed text-sm mt-4"
      >
